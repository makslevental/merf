...
class UnetN2N(nn.Module):
    ...
    def forward(self, x):
        self.enc_conv0(x)
        out_pool1 = self.pool1(
            self.enc_relu1(self.enc_conv1(self.enc_relu0(self.enc_conv0(x))))
        )
        ...
        out = self.upsample4(
            self.dec_relu5b(
                self.dec_conv5b(
                    self.dec_relu5a(self.dec_conv5a(torch.cat((out, out_pool4), 1)))
                )
            )
        )
        out = self.upsample3(
            self.dec_relu4b(
                self.dec_conv4b(
                    self.dec_relu4a(self.dec_conv4a(torch.cat((out, out_pool3), 1)))
                )
            )
        )
        out = self.upsample2(
            self.dec_relu3b(
                self.dec_conv3b(
                    self.dec_relu3a(self.dec_conv3a(torch.cat((out, out_pool2), 1)))
                )
            )
        )
        out = self.upsample1(
            self.dec_relu2b(
                self.dec_conv2b(
                    self.dec_relu2a(self.dec_conv2a(torch.cat((out, out_pool1), 1)))
                )
            )
        )
        out = self.dec_conv1c(
            self.dec_relu1b(
                self.dec_conv1b(
                    self.dec_relu1a(self.dec_conv1a(torch.cat((out, x), 1)))
                )
            )
        )
        return out
...
class UnetN2Nv2(nn.Module):
    ...
    def forward(self, x):
        out_pool1 = self.pool1(
            self.enc_relu1(
                self.enc_bn1(self.enc_conv1(self.enc_relu0(self.enc_conv0(x))))
            )
        )
            ...
            self.dec_relu5b(
                self.dec_bn5b(
                    self.dec_conv5b(
                        self.dec_relu5a(
                            self.dec_bn5a(
                                self.dec_conv5a(torch.cat((out, out_pool4), 1))
                            )
                        )
                    )
                )
            ...
            self.dec_relu4b(
                self.dec_bn4b(
                    self.dec_conv4b(
                        self.dec_relu4a(
                            self.dec_bn4a(
                                self.dec_conv4a(torch.cat((out, out_pool3), 1))
                            )
                        )
                    )
                )
            ...
            self.dec_relu3b(
                self.dec_bn3b(
                    self.dec_conv3b(
                        self.dec_relu3a(
                            self.dec_bn3a(
                                self.dec_conv3a(torch.cat((out, out_pool2), 1))
                            )
                        )
                    )
                )
            ...
            self.dec_relu2b(
                self.dec_bn2b(
                    self.dec_conv2b(
                        self.dec_relu2a(
                            self.dec_bn2a(
                                self.dec_conv2a(torch.cat((out, out_pool1), 1))
                            )
                        )
                    )
                )
        ...
        out = self.dec_conv1c(
            self.dec_relu1b(
                self.dec_bn1b(
                    self.dec_conv1b(
                        self.dec_relu1a(
                            self.dec_bn1a(self.dec_conv1a(torch.cat((out, x), 1)))
                        )
                    )
                )
            )
    ...
    def _model_size(self):
        ...
        for module in self.modules():
            if (
                "Conv" in module.__class__.__name__
